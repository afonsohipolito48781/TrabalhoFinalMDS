...
public class Quarto {
    ...
    // Issue #?: adicionado "boolean emManutenção"
    private boolean emManutencao = false;
    // ---

    private List<Manutencao> registoManutencoes = new ArrayList<>();

    // Construtores
    public Quarto(String tipoVista, int capacidade, int camas, int wcs, boolean cozinha, boolean varanda) {
        // Restrições
        if (tipoVista == null ){
            throw new IllegalArgumentException("Tipo de vista nulo.");
        }
        if (capacidade <= 0) {
    ...
    public Quarto(int id_quarto, String tipoVista, int capacidade, int camas, int wcs, boolean cozinha, boolean varanda) {
        ...
        if (id_quarto < 1){
            throw new IllegalArgumentException("Quarto não existente.");
        }
        if (capacidade <= 0) {
            throw new IllegalArgumentException("Capacidade inválida.");
        }
